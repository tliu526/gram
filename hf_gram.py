...
def init_params(options):
    ...
    params['W_output'] = get_random_weight(hiddenDimSize, 1)
    params['b_output'] = np.zeros(1).astype(config.floatX)
...
def build_model(tparams, leavesList, ancestorsList, options):
    ...
    # axis 2 is the number of output classes (in the HF case, 1).
...
def load_data(seqFile, labelFile, outfile, timeFile=''):
    ...
    pickle.dump(test_set, open(outfile + '.test_set', 'wb'), -1)
...
def train_GRAM(
    ...
    for epoch in xrange(max_epochs):
        ...
        if validCost < bestValidCost:
            ...
            pickle.dump(predLabels, open(outFile + '_' + str(epoch) + '.test_probs', 'wb'), -1)
